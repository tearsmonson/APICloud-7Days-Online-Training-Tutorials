/*
此处是一个示例,展示如何基于 apiruntime 来重写特定的 APICloud 引擎
或模块的方法.
*/

/*
函数内,如果不想处理特定模块,直接调用 next 函数即可.

当 frame, window, runtime 级别同时实现此函数时,
加载优先级为: frame > window > runtime > 内置默认实现
...
@payload:
  ...
  @apiDom     runtime 所在页面的 window dom 对象.
...
@return: 不作处理.如果不想处理某个模块方法, 应该显式返回字符串 'TO_NEXT_API_RUNTIME',
          以便往上传播调用.
*/
function apiruntime(payload) {
